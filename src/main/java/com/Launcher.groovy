...
class Launcher {
    ...
    static void main(String[] args) {
        ...
        final GraphFrame graph = new GraphFrame("Best Computed and Original");
        graph.pack();
        RefineryUtilities.centerFrameOnScreen(graph);
        graph.setVisible(true);

        Launcher main = new Launcher(xx, dd, RULES_COUNT, { List yAndDList, List fAndDList ->
            graph.refresh(yAndDList, fAndDList)
        })
        main.train()
    }

    Launcher(List<List<Double>> x, List<Double> d, int rulesCount, Closure callback) {
        this.x = x
        this.d = d
    ...
    boolean doTrain() {
        ...
        while (minTotalValidationAverageError > MAX_ALLOWED_AVERAGE_ERROR) {
            ...
            if (totalValidationAverageError < minTotalValidationAverageError) {
                ...
                callback.call(bestYAndD, computed)
            }
